  ...
  return (
    <section className="dashboard-page">
      <aside className="dashboard-sidebar">
        <div><span className="eyebrow">Control room</span><h1>{user.name}</h1><p>{user.email}</p></div>
        <nav><Link className="active" href="/dashboard">Overview</Link><Link href="/dashboard/claims">My claims</Link><Link href="/join">New company entry</Link>{["ADMIN", "MODERATOR"].includes(user.role) && <Link href="/admin">Moderation</Link>}</nav>
        <SignOutButton />
      </aside>
      <div className="dashboard-content">
        ...
        <div className="dashboard-stats"><div><span>Companies</span><strong>{memberships.length}</strong></div><div><span>Published deals</span><strong>{deals.length}</strong></div><div><span>Credits claimed</span><strong>{claims.length}</strong></div></div>
