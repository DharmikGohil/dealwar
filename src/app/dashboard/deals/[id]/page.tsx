...
export default async function DashboardDealPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const paid = deal.payments.some((payment) => payment.status === "SUCCEEDED");
  return (
    <section className="deal-status-page">
      ...
      <div className="gate-grid">
        <div className={paid ? "gate done" : "gate"}><CircleDollarSign /><span>Entry payment</span><strong>{paid ? "Paid" : "Pending"}</strong><small>{formatMoney(deal.entryFeeCents)}</small></div>
