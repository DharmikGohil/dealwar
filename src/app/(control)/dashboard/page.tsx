...
export default async function DashboardPage() {
  ...
  const entryCount = memberships.reduce(
    (total, membership) => total + membership.organization.products.reduce((productTotal, product) => productTotal + product.deals.length, 0),
    0,
  );
  return (
    <section className="dashboard-content">
      ...
      <div className="dashboard-stats"><div><span>Companies</span><strong>{memberships.length}</strong></div><div><span>Total entries</span><strong>{entryCount}</strong></div><div><span>Credits claimed</span><strong>{claims.length}</strong></div></div>
